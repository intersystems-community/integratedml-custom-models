...
class BehavioralFeatureEngineer:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.is_fitted:
            raise ValueError(
                "Behavioral feature engineer must be fitted before transform"
            )
        ...
        X_features = X_features.sort_values(["customer_id", "timestamp"]).reset_index(
            drop=True
        )
    ...
    def _build_customer_profiles(
        ...
        for customer_id, group in X.groupby("customer_id"):
            ...
            if "timestamp" in group.columns:
                ...
                days_active = (
                    group_sorted["timestamp"].max() - group_sorted["timestamp"].min()
                ).days
                profile["temporal"] = {
                ...
                profile["temporal"][
                    "dow_distribution"
                ] = days_of_week.value_counts().to_dict()
            ...
            if "merchant_name" in group.columns:
                ...
                profile["merchant"] = {
                    ...
                    "merchant_loyalty": merchant_counts.max()
                    / len(group),  # Fraction at most frequent merchant
                    "merchant_diversity": len(merchant_counts)
                    / len(group),  # Merchant variety
                }
    ...
    def _compute_behavioral_baselines(self, X: pd.DataFrame) -> None:
        ...
        if "timestamp" in X.columns:
            ...
            self.temporal_baselines = {
                "global_hour_distribution": hours.value_counts(
                    normalize=True
                ).to_dict(),
    ...
    def _add_spending_pattern_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for customer_id, group_indices in X.groupby("customer_id").groups.items():
            ...
            for idx in group_indices:
                ...
                if personal_std > 0:
                    X.loc[idx, "amount_vs_personal_std"] = (
                        amount - personal_avg
                    ) / personal_std
        ...
        for customer_id, group in X.groupby("customer_id"):
            ...
            for i, (idx, row) in enumerate(group.iterrows()):
                ...
                recent_mask = (group["timestamp"] >= recent_window) & (
        ...
        for customer_id, group_indices in X.groupby("customer_id").groups.items():
            ...
            for idx in group_indices:
                ...
                if hour_dist:
                    ...
                    X.loc[idx, "hour_deviation_score"] = 1 - (
                        current_hour_freq / max_hour_freq
                    )
                ...
                if dow_dist:
                    ...
                    X.loc[idx, "dow_deviation_score"] = 1 - (
                        current_dow_freq / max_dow_freq
                    )
    ...
    def _add_merchant_pattern_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for customer_id, group_indices in X.groupby("customer_id").groups.items():
            ...
            for idx in group_indices:
                ...
                if merchant in top_merchants:
                    ...
                    X.loc[idx, "merchant_novelty_score"] = (
                        merchant_freq / total_transactions
                    )
        ...
        if "mcc" in X.columns:
            ...
            for customer_id, group_indices in X.groupby("customer_id").groups.items():
                ...
                merchant_profile = self.customer_profiles[customer_id].get(
                    "merchant", {}
                )
        ...
        for customer_id, group_indices in X.groupby("customer_id").groups.items():
            ...
            for idx in group_indices:
                ...
                if first_transaction:
                    ...
                    X.loc[idx, "days_since_first_transaction"] = max(
                        days_since_first, 0
                    )
                ...
                customer_data = customer_data[
                    customer_data["timestamp"] < transaction_time
                ]
    ...
    def _add_deviation_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if available_features:
            ...
            X["high_behavioral_deviation"] = (
                X["behavioral_deviation_score"] > 0.7
            ).astype(int)
            X["moderate_behavioral_deviation"] = (
                (X["behavioral_deviation_score"] > 0.4)
                & (X["behavioral_deviation_score"] <= 0.7)
            ).astype(int)
    ...
    def get_feature_names(self) -> List[str]:
        ...
        features.extend(
            [
                ...
                "moderate_behavioral_deviation",
                "pattern_break_count",
                "multiple_pattern_breaks",
            ]
        )
    ...
    def get_behavioral_insights(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "behavioral_deviation_score" in X.columns:
            insights["behavioral_analysis"] = {
                ...
                "moderate_deviation_pct": X["moderate_behavioral_deviation"].mean()
                * 100,
            }
        ...
        if "customer_activity_level" in X.columns:
            activity_dist = (
                X["customer_activity_level"].value_counts(normalize=True) * 100
            )
