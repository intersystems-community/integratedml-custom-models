...
class RiskFeatureEngineer:
    ...
    def fit(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> 'RiskFeatureEngineer':
        """
    ...
    def _learn_fraud_patterns(self, X: pd.DataFrame, y: np.ndarray) -> None:
        ...
        velocity_features = [col for col in X.columns if 'velocity' in col or 'rapid_fire' in col]
        if velocity_features:
            ...
            self.fraud_patterns['velocity'] = {
                ...
                'risk_multipliers': (fraud_velocity / (legit_velocity + 0.001)).to_dict()
            }
        
        # High-risk location patterns
        location_features = [col for col in X.columns if 'location_risk' in col or 'impossible_travel' in col]
        ...
        behavioral_features = [col for col in X.columns if 'behavioral' in col or 'deviation' in col]
        if behavioral_features:
            ...
            self.fraud_patterns['behavioral'] = {
                ...
                'deviation_thresholds': (fraud_behavioral * 0.8).to_dict()  # Conservative threshold
            }
    ...
    def _compute_risk_statistics(self, X: pd.DataFrame) -> None:
        ...
        if 'amount' in X.columns:
            self.risk_statistics['amount'] = {
                'mean': X['amount'].mean(),
                'std': X['amount'].std(),
                'percentiles': X['amount'].quantile([0.5, 0.75, 0.9, 0.95, 0.99]).to_dict()
            }
    ...
    def _add_transaction_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if consistency_features:
            X['transaction_consistency_risk'] = 1.0 - X[consistency_features].mean(axis=1)
        else:
            X['transaction_consistency_risk'] = 0.0
        
        # Combined transaction risk
        transaction_risk_features = [
            'amount_risk_score', 'round_amount_risk', 'payment_method_risk',
            'merchant_risk', 'transaction_consistency_risk'
        ]
    ...
    def _add_velocity_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # High velocity risk
        velocity_features = [col for col in X.columns if 'velocity' in col and 'rapid_fire' not in col]
        if velocity_features:
            velocity_risks = []
        ...
        velocity_risk_features = [
            'rapid_fire_risk_score', 'velocity_risk_score', 'frequency_anomaly_risk'
        ]
    ...
    def _add_location_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'combined_location_risk' in X.columns:
            ...
            location_risk_features = [col for col in X.columns if 'location_risk' in col]
        ...
        if 'timezone_mismatch' in X.columns:
            ...
            if 'timezone_difference' in X.columns:
                ...
                X['timezone_risk_score'] = np.maximum(
                    X['timezone_risk_score'],
                    normalized_tz_diff * 0.5
                )
        else:
        ...
        location_risk_features = [
            'geographic_risk_score', 'travel_risk_score', 
            'distance_anomaly_risk', 'timezone_risk_score'
        ]
    ...
    def _add_behavioral_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        spending_risk_features = [col for col in X.columns if any(
            keyword in col for keyword in ['unusual_amount', 'spending_burst', 'very_large_amount']
        )]
        ...
        behavioral_risk_features = [
            'behavioral_risk_score', 'pattern_break_risk', 'new_customer_risk',
            'dormant_reactivation_risk', 'spending_pattern_risk', 'merchant_pattern_risk'
        ]
    ...
    def _add_ensemble_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if risk_components:
            ...
            if total_weight > 0:
                ...
                X['ensemble_risk_score'] = np.mean(list(risk_components.values()), axis=0)
        else:
    ...
    def _add_pattern_matching_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'amount_vs_personal_avg' in X.columns and 'amount_ratio' in self.fraud_patterns:
            amount_pattern = self.fraud_patterns['amount_ratio']
            risk_threshold = amount_pattern.get('risk_threshold', 2.0)
            
            amount_risk_matches = (X['amount_vs_personal_avg'] >= risk_threshold).astype(int)
            ...
            for feature, risk_multiplier in velocity_patterns.items():
                if feature in X.columns and risk_multiplier > 1.5:
                    ...
                    if feature_max > 0:
                        ...
                        pattern_score += normalized_values * (risk_multiplier - 1) / 5  # Normalize contribution
                        matched_patterns += 1
        ...
        if 'location' in self.fraud_patterns:
            ...
            if total_indicators > 0:
                X['fraud_pattern_similarity'] += (location_matches / total_indicators) * 0.25
        
        # Behavioral pattern matching
        if 'behavioral' in self.fraud_patterns:
            behavioral_thresholds = self.fraud_patterns['behavioral']['deviation_thresholds']
            ...
            if total_thresholds > 0:
                X['fraud_pattern_similarity'] += (behavioral_matches / total_thresholds) * 0.25
        
        # Known fraud pattern detection
        X['known_fraud_pattern_match'] = (X['fraud_pattern_similarity'] > 0.6).astype(int)
    ...
    def _add_risk_categorization(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Risk flags
        if 'ensemble_risk_score' in X.columns:
            X['high_risk_flag'] = (X['ensemble_risk_score'] > self.risk_thresholds['high_risk']).astype(int)
            X['medium_risk_flag'] = (
                (X['ensemble_risk_score'] > self.risk_thresholds['medium_risk']) &
                (X['ensemble_risk_score'] <= self.risk_thresholds['high_risk'])
            ).astype(int)
            X['low_risk_flag'] = (X['ensemble_risk_score'] <= self.risk_thresholds['low_risk']).astype(int)
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.enable_transaction_risk:
            features.extend([
                'amount_risk_score', 'round_amount_risk', 'payment_method_risk',
                'merchant_risk', 'transaction_consistency_risk', 'transaction_risk_score'
            ])
        ...
        if self.enable_location_risk:
            features.extend([
                'geographic_risk_score', 'travel_risk_score', 'distance_anomaly_risk',
                'timezone_risk_score', 'location_risk_combined'
            ])
        ...
        if self.enable_ensemble_risk:
            features.extend([
                'ensemble_risk_score', 'risk_score_variance', 'risk_score_consensus',
                'max_component_risk', 'min_component_risk', 'risk_score_range'
            ])
        
        # Pattern matching features
        features.extend([
            'fraud_pattern_similarity', 'known_fraud_pattern_match'
        ])
        
        # Risk categorization features
        features.extend([
            'risk_level', 'high_risk_flag', 'medium_risk_flag', 'low_risk_flag',
            'risk_confidence', 'high_confidence_assessment', 'low_confidence_assessment'
        ])
    ...
    def get_risk_summary(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        risk_components = ['transaction_risk_score', 'velocity_risk_combined', 
                          'location_risk_combined', 'behavioral_risk_combined']
    ...
    def compute_realtime_risk(self, transaction_features: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        """
        if not self.is_fitted:
            raise ValueError("Risk feature engineer must be fitted before real-time computation")
