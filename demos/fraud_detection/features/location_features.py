...
class LocationFeatureEngineer:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.is_fitted:
            raise ValueError(
                "Location feature engineer must be fitted before transform"
            )
    ...
    def _add_basic_location_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "customer_location" in X.columns:
            X["customer_location_encoded"] = self._encode_location(
                X["customer_location"]
            )
        ...
        if "merchant_location" in X.columns:
            X["merchant_location_encoded"] = self._encode_location(
                X["merchant_location"]
            )
        ...
        if "customer_location" in X.columns and "merchant_location" in X.columns:
            X["same_location"] = (
                X["customer_location"] == X["merchant_location"]
            ).astype(int)
            X["same_state"] = (X["customer_state"] == X["merchant_state"]).astype(int)
            X["same_country"] = (X["customer_country"] == X["merchant_country"]).astype(
                int
            )
    ...
    def _add_distance_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "customer_location" not in X.columns or "merchant_location" not in X.columns:
            logger.warning(
                "Customer or merchant location missing, skipping distance features"
            )
            return X
        ...
        # Distance flags
        X["is_local_transaction"] = (X["customer_merchant_distance_km"] <= 25).astype(
            int
        )
        X["is_long_distance"] = (X["customer_merchant_distance_km"] > 500).astype(int)
        X["is_very_long_distance"] = (X["customer_merchant_distance_km"] > 1000).astype(
            int
        )
            ...
            for i in range(1, len(group)):
                ...
                if "customer_location" in X.columns:
                    distance = self._calculate_distance(
                        prev_row["customer_location"], current_row["customer_location"]
                    )
                    X.loc[current_idx, "travel_distance_km"] = distance

                    # Calculate travel time
                    time_diff = (
                        current_row["timestamp"] - prev_row["timestamp"]
    ...
    def _add_location_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "customer_location" in X.columns and self.location_risk_scores:
            X["customer_location_risk"] = (
                X["customer_location"].map(self.location_risk_scores).fillna(0.5)
            )  # Default medium risk

        # Merchant location risk
        if "merchant_location" in X.columns and self.location_risk_scores:
            X["merchant_location_risk"] = (
                X["merchant_location"].map(self.location_risk_scores).fillna(0.5)
            )

        # State-level risk
        if "customer_state" in X.columns and self.state_risk_scores:
            X["customer_state_risk"] = (
                X["customer_state"].map(self.state_risk_scores).fillna(0.5)
            )

        if "merchant_state" in X.columns and self.state_risk_scores:
            X["merchant_state_risk"] = (
                X["merchant_state"].map(self.state_risk_scores).fillna(0.5)
            )

        # Country-level risk
        if "customer_country" in X.columns and self.country_risk_scores:
            X["customer_country_risk"] = (
                X["customer_country"].map(self.country_risk_scores).fillna(0.5)
            )
        ...
        if "customer_location_risk" in X.columns:
            X["high_risk_customer_location"] = (
                X["customer_location_risk"] > 0.7
            ).astype(int)

        if "merchant_location_risk" in X.columns:
            X["high_risk_merchant_location"] = (
                X["merchant_location_risk"] > 0.7
            ).astype(int)
    ...
    def _add_timezone_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "merchant_location" in X.columns:
            ...
            X["merchant_local_hour"] = X.apply(
                lambda row: self._convert_to_local_time(
                    row["timestamp"], row["merchant_timezone"]
                ),
                axis=1,
            )
        ...
        if "customer_location" in X.columns:
            ...
            X["customer_local_hour"] = X.apply(
                lambda row: self._convert_to_local_time(
                    row["timestamp"], row["customer_timezone"]
                ),
                axis=1,
            )
        ...
        if "merchant_timezone" in X.columns and "customer_timezone" in X.columns:
            X["timezone_mismatch"] = (
                X["merchant_timezone"] != X["customer_timezone"]
            ).astype(int)
            X["timezone_difference"] = X.apply(
                lambda row: abs(
                    self._get_timezone_offset(row["merchant_timezone"])
    ...
    def _compute_location_risk_scores(self, X: pd.DataFrame, y: np.ndarray) -> None:
        ...
        if "customer_location" in X.columns:
            location_fraud_rates = (
                pd.DataFrame({"customer_location": X["customer_location"], "fraud": y})
                .groupby("customer_location")["fraud"]
                .agg(["mean", "count"])
            )

            # Only consider locations with sufficient data
            min_transactions = 5
            reliable_locations = location_fraud_rates[
                location_fraud_rates["count"] >= min_transactions
            ]
        ...
        if "customer_location" in X.columns:
            ...
            state_fraud_rates = (
                pd.DataFrame({"state": states, "fraud": y})
                .groupby("state")["fraud"]
                .agg(["mean", "count"])
            )
        ...
        if "customer_location" in X.columns:
            ...
            country_fraud_rates = (
                pd.DataFrame({"country": countries, "fraud": y})
                .groupby("country")["fraud"]
                .agg(["mean", "count"])
            )
    ...
    def _build_location_coordinates(self, X: pd.DataFrame) -> None:
        ...
        for location in unique_locations:
            self.location_coordinates[location] = self._get_simplified_coordinates(
                location
            )
    ...
    def _get_timezone(self, location: str) -> str:
        ...
        if any(
            city in location_lower for city in ["new york", "philadelphia", "atlanta"]
        ):
            return "EST"
            ...
            for city in ["los angeles", "san francisco", "seattle"]
        ):
            return "PST"
        else:
            return "UTC"
    ...
    def _get_timezone_offset(self, timezone: str) -> int:
        ...
        """
        offsets = {"EST": -5, "CST": -6, "MST": -7, "PST": -8, "UTC": 0}
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.enable_location_risk:
            features.extend(
                [
                    "customer_location_risk",
                    "merchant_location_risk",
                    "customer_state_risk",
                    "merchant_state_risk",
                    "customer_country_risk",
                    "combined_location_risk",
                    "high_risk_customer_location",
                    "high_risk_merchant_location",
                ]
            )
        ...
        if self.enable_timezone_features:
            features.extend(
                [
                    ...
                    "merchant_after_hours",
                    "merchant_late_night",
                    "customer_timezone",
                    "customer_local_hour",
                    "customer_unusual_hour",
                    "timezone_mismatch",
                    "timezone_difference",
                ]
            )
    ...
    def get_location_insights(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "combined_location_risk" in X.columns:
            insights["risk_analysis"] = {
                ...
                "high_risk_locations_pct": (X["combined_location_risk"] > 0.7).mean()
                * 100,
            }
