...
class TransactionFeatureEngineer:
    ...
    def __init__(
        ...
        """
        self.amount_buckets = amount_buckets or [
            0,
            10,
            50,
            100,
    ...
    def _add_amount_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # Amount buckets
        X["amount_bucket"] = pd.cut(
            X["amount"], bins=self.amount_buckets, labels=False, include_lowest=True
        )
        ...
        # Amount patterns
        X["amount_decimal_places"] = X["amount"].apply(
    ...
    def _add_merchant_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            X["merchant_encoded"] = merchant_filled.apply(
                lambda x: (
                    self.merchant_encoder.transform([x])[0]
                    if x in self.merchant_encoder.classes_
                    else -1
                )
            )
        ...
        X["merchant_has_numbers"] = (
            X["merchant_name"].fillna("").str.contains(r"\d").astype(int)
        )
        X["merchant_has_special_chars"] = (
            X["merchant_name"].fillna("").str.contains(r"[^a-zA-Z0-9\s]").astype(int)
        )
    ...
    def _add_payment_method_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "payment_method" not in X.columns:
            logger.warning(
                "Payment method column not found, skipping payment method features"
            )
            return X
        ...
        try:
            X["payment_method_encoded"] = self.payment_method_encoder.transform(
                payment_filled
            )
        except ValueError:
            ...
            X["payment_method_encoded"] = payment_filled.apply(
                lambda x: (
                    self.payment_method_encoder.transform([x])[0]
                    if x in self.payment_method_encoder.classes_
                    else -1
                )
            )
        ...
        X["is_card_payment"] = (
            X["payment_method"].fillna("").str.contains("card", case=False).astype(int)
        )
        X["is_digital_payment"] = (
            X["payment_method"]
            .fillna("")
            .str.contains("digital|paypal|venmo|zelle", case=False)
            .astype(int)
        )
        X["is_cash_equivalent"] = (
            X["payment_method"]
            .fillna("")
            .str.contains("cash|check|wire", case=False)
            .astype(int)
        )
    ...
    def _add_cross_validation_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "amount" in X.columns and "merchant_name" in X.columns:
            ...
            X["amount_merchant_consistency"] = self._check_amount_merchant_consistency(
                X
            )
    ...
    def _add_risk_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "merchant_name" in X.columns and self.merchant_risk_scores:
            X["merchant_risk_score"] = (
                X["merchant_name"].map(self.merchant_risk_scores).fillna(0.5)
            )
        ...
        if "payment_method" in X.columns and self.payment_method_risk_scores:
            X["payment_method_risk_score"] = (
                X["payment_method"].map(self.payment_method_risk_scores).fillna(0.5)
            )
    ...
    def _compute_risk_scores(self, X: pd.DataFrame, y: np.ndarray) -> None:
        ...
        if "merchant_name" in X.columns:
            merchant_fraud_rates = (
                X.groupby("merchant_name")["fraud"].mean()
                if "fraud" in X.columns
                else pd.DataFrame({"merchant_name": X["merchant_name"], "fraud": y})
                .groupby("merchant_name")["fraud"]
                .mean()
            )
        ...
        if "mcc" in X.columns:
            mcc_fraud_rates = (
                X.groupby("mcc")["fraud"].mean()
                if "fraud" in X.columns
                else pd.DataFrame({"mcc": X["mcc"], "fraud": y})
                .groupby("mcc")["fraud"]
                .mean()
            )
        ...
        if "payment_method" in X.columns:
            payment_fraud_rates = (
                X.groupby("payment_method")["fraud"].mean()
                if "fraud" in X.columns
                else pd.DataFrame({"payment_method": X["payment_method"], "fraud": y})
                .groupby("payment_method")["fraud"]
                .mean()
            )
    ...
    def _check_amount_merchant_consistency(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if "amount" in X.columns and "merchant_name" in X.columns:
            ...
            convenience_mask = X["merchant_name"].str.contains(
                "convenience|7-eleven|circle k", case=False, na=False
            )
            high_amount_mask = X["amount"] > 500
            scores[convenience_mask & high_amount_mask] = 0.3

            # Very low amounts at luxury stores
            luxury_mask = X["merchant_name"].str.contains(
                "luxury|gucci|tiffany", case=False, na=False
            )
