...
class VelocityFeatureEngineer:
    ...
    def fit(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> 'VelocityFeatureEngineer':
        """
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.is_fitted:
            raise ValueError("Velocity feature engineer must be fitted before transform")
    ...
    def transform_realtime(self, transaction: Dict[str, Any]) -> Dict[str, float]:
        ...
        """
        if not self.is_fitted:
            raise ValueError("Velocity feature engineer must be fitted before real-time transform")
    ...
    def _compute_baselines(self, X: pd.DataFrame, y: Optional[np.ndarray] = None) -> None:
        """
        Compute baseline statistics from training data.
        ...
        # Global baselines
        self.global_baselines = {
            'avg_transaction_count_per_hour': len(X) / max(1, (X['timestamp'].max() - X['timestamp'].min()).total_seconds() / 3600),
        ...
        if 'customer_id' in X.columns:
            ...
            customer_stats.columns = ['customer_id', 'transaction_count', 'avg_amount', 
                                    'std_amount', 'first_transaction', 'last_transaction']
            customer_stats['days_active'] = (customer_stats['last_transaction'] - 
                                           customer_stats['first_transaction']).dt.total_seconds() / 86400
            customer_stats['transactions_per_day'] = customer_stats['transaction_count'] / np.maximum(customer_stats['days_active'], 1)
            
            self.customer_baselines = customer_stats.set_index('customer_id').to_dict('index')
        
        # Merchant baselines
        if 'merchant_name' in X.columns:
            merchant_stats = X.groupby('merchant_name').agg({
                'amount': ['count', 'mean', 'std'],
                'timestamp': ['min', 'max']
            }).reset_index()
            
            merchant_stats.columns = ['merchant_name', 'transaction_count', 'avg_amount',
                                    'std_amount', 'first_transaction', 'last_transaction']
            merchant_stats['days_active'] = (merchant_stats['last_transaction'] - 
                                           merchant_stats['first_transaction']).dt.total_seconds() / 86400
            merchant_stats['transactions_per_day'] = merchant_stats['transaction_count'] / np.maximum(merchant_stats['days_active'], 1)
            
            self.merchant_baselines = merchant_stats.set_index('merchant_name').to_dict('index')
        ...
        for customer_id, group in X.groupby('customer_id'):
            ...
            for i, (idx, row) in enumerate(group.iterrows()):
                ...
                for window in self.time_windows:
                    ...
                    window_mask = (group['timestamp'] >= start_time) & (group['timestamp'] < current_time)
                    ...
                    if len(window_txns) > 0:
                        X.loc[idx, f'customer_amount_sum_{window}m'] = window_txns['amount'].sum()
                        X.loc[idx, f'customer_amount_avg_{window}m'] = window_txns['amount'].mean()
                        
                        if 'merchant_name' in window_txns.columns:
                            X.loc[idx, f'customer_unique_merchants_{window}m'] = window_txns['merchant_name'].nunique()
        
        # Add derived velocity features
        for window in self.time_windows:
            # Transaction frequency (transactions per minute)
            X[f'customer_tx_frequency_{window}m'] = X[f'customer_tx_count_{window}m'] / window
            
            # Amount velocity (amount per minute)
            X[f'customer_amount_velocity_{window}m'] = X[f'customer_amount_sum_{window}m'] / window
            ...
            if self.customer_baselines:
                ...
                for customer_id in X['customer_id']:
                    baseline = self.customer_baselines.get(customer_id, {}).get('transactions_per_day', 1)
                    expected_count = baseline * (window / 1440)  # Convert to expected count for window
                    baseline_values.append(expected_count)
                
                X[f'customer_velocity_vs_baseline_{window}m'] = X[f'customer_tx_count_{window}m'] / np.maximum(baseline_values, 0.1)
        ...
        for merchant_name, group in X.groupby('merchant_name'):
            ...
            for i, (idx, row) in enumerate(group.iterrows()):
                ...
                for window in self.time_windows:
                    ...
                    if len(window_txns) > 0:
                        X.loc[idx, f'merchant_amount_sum_{window}m'] = window_txns['amount'].sum()
                        
                        if 'customer_id' in window_txns.columns:
                            X.loc[idx, f'merchant_unique_customers_{window}m'] = window_txns['customer_id'].nunique()
        
        # Add derived merchant velocity features
        for window in self.time_windows:
            # Merchant transaction frequency
            X[f'merchant_tx_frequency_{window}m'] = X[f'merchant_tx_count_{window}m'] / window
            
            # Merchant amount velocity
            X[f'merchant_amount_velocity_{window}m'] = X[f'merchant_amount_sum_{window}m'] / window
            
            # Customer diversity ratio
            X[f'merchant_customer_diversity_{window}m'] = (
                X[f'merchant_unique_customers_{window}m'] / np.maximum(X[f'merchant_tx_count_{window}m'], 1)
            )
            ...
            for window in [5, 15, 60]:
                for i, (idx, row) in enumerate(group.iterrows()):
                    ...
                    if len(window_amounts) > 0:
                        ...
                        X.loc[idx, f'amount_rolling_std_{window}m'] = rolling_std if not pd.isna(rolling_std) else 0
                        X.loc[idx, f'amount_vs_rolling_mean_{window}m'] = row['amount'] / max(rolling_mean, 0.01)
    ...
    def _add_location_velocity_features(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X['location_changed'] = (X['customer_location'] != X['prev_location']).astype(int)
            ...
            for window in [15, 60, 240]:
                for i, (idx, row) in enumerate(group.iterrows()):
                    ...
                    if len(window_txns) > 0:
                        ...
                        location_changes = (window_txns['customer_location'] != window_txns['customer_location'].shift(1)).sum()
                        X.loc[idx, f'location_changes_{window}m'] = location_changes
                        
                        # Count unique locations
                        X.loc[idx, f'unique_locations_{window}m'] = window_txns['customer_location'].nunique()
        
        # Location velocity ratios
        X['location_velocity_15m'] = X['location_changes_15m'] / 15 * 60  # changes per hour
        X['location_velocity_60m'] = X['location_changes_60m'] / 60 * 60  # changes per hour
        ...
        if self.customer_velocity and self.merchant_velocity:
            for window in [5, 15, 60]:
                ...
                if customer_col in X.columns and merchant_col in X.columns:
                    X[f'customer_merchant_velocity_ratio_{window}m'] = (
                        X[customer_col] / np.maximum(X[merchant_col], 1)
                    )
            ...
            if f'customer_tx_count_{window}m' in X.columns:
                ...
                if std_count > 0:
                    X[f'customer_velocity_zscore_{window}m'] = (
                        (X[f'customer_tx_count_{window}m'] - mean_count) / std_count
                    )
                else:
    ...
    def _compute_realtime_customer_velocity(self, customer_id: str, timestamp: datetime, 
                                          amount: float, location: str) -> Dict[str, float]:
        """
        ...
        for window in self.time_windows:
            ...
            window_txns = [txn for txn in customer_txns if txn['timestamp'] >= start_time]
            
            # Compute velocity features
            features[f'customer_tx_count_{window}m'] = len(window_txns)
            features[f'customer_amount_sum_{window}m'] = sum(txn['amount'] for txn in window_txns)
            features[f'customer_tx_frequency_{window}m'] = len(window_txns) / window
            
            if window_txns:
                features[f'customer_amount_avg_{window}m'] = features[f'customer_amount_sum_{window}m'] / len(window_txns)
                features[f'customer_unique_merchants_{window}m'] = len(set(txn.get('merchant_name', '') for txn in window_txns))
            else:
    ...
    def _compute_realtime_merchant_velocity(self, merchant_name: str, timestamp: datetime, 
        ...
        for window in self.time_windows:
            ...
            # Filter transactions in window
            window_txns = [txn for txn in merchant_txns if txn['timestamp'] >= start_time]
            
            # Compute velocity features
            features[f'merchant_tx_count_{window}m'] = len(window_txns)
            features[f'merchant_amount_sum_{window}m'] = sum(txn['amount'] for txn in window_txns)
            features[f'merchant_tx_frequency_{window}m'] = len(window_txns) / window
            
            if window_txns:
                features[f'merchant_unique_customers_{window}m'] = len(set(txn.get('customer_id', '') for txn in window_txns))
            else:
    ...
    def _update_transaction_caches(self, transaction: Dict[str, Any], timestamp: datetime) -> None:
        """
        ...
        if customer_id:
            ...
            if len(self.customer_transactions[customer_id]) > self.cache_size:
                self.customer_transactions[customer_id] = self.customer_transactions[customer_id][-self.cache_size:]
        ...
        if merchant_name:
            ...
            if len(self.merchant_transactions[merchant_name]) > self.cache_size:
                self.merchant_transactions[merchant_name] = self.merchant_transactions[merchant_name][-self.cache_size:]
        ...
        if location:
            ...
            if len(self.location_transactions[location]) > self.cache_size:
                self.location_transactions[location] = self.location_transactions[location][-self.cache_size:]
    ...
    def get_feature_names(self) -> List[str]:
        ...
        if self.merchant_velocity:
            ...
            features.extend([
                'merchant_rapid_fire_1m',
                'merchant_rapid_fire_5m'
            ])
        
        # Amount velocity features
        if self.amount_velocity:
            features.extend([
                'prev_amount', 'amount_change', 'amount_change_ratio', 'amount_change_abs',
                'amount_increase', 'amount_large_increase', 'amount_large_decrease'
            ])
        ...
        if self.location_velocity:
            features.extend([
                ...
                'location_velocity_15m', 'location_velocity_60m',
                'potential_impossible_travel'
            ])
            
            for window in [15, 60, 240]:
                features.extend([
                    f'location_changes_{window}m',
                    f'unique_locations_{window}m'
                ])
    ...
    def get_velocity_summary(self, X: pd.DataFrame) -> Dict[str, Any]:
        ...
        if self.merchant_velocity:
            merchant_rapid_fire_1m = X.get('merchant_rapid_fire_1m', pd.Series([])).sum()
            merchant_rapid_fire_5m = X.get('merchant_rapid_fire_5m', pd.Series([])).sum()
