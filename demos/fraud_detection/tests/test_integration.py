...
class TestEndToEndWorkflow:
    ...
    def test_complete_fraud_detection_pipeline(
        self, transaction_generator, test_config
    ):
    ...
    def test_real_time_transaction_processing(self, transaction_generator):
        ...
        for idx, transaction in transaction_stream.iterrows():
            ...
            ), f"Transaction {idx} processing time {processing_time_ms:.2f}ms too slow"
    ...
    def test_caching_integration(self, transaction_generator):
        ...
        test_transaction = (
            transaction_generator.generate_transactions(n_transactions=1)
            .iloc[0]
            .to_dict()
        )
        ...
        start_time = time.perf_counter()
        features_1 = feature_processor.transform_single(test_transaction)
        first_time = (time.perf_counter() - start_time) * 1000

        # Second processing (cache hit)
        start_time = time.perf_counter()
        features_2 = feature_processor.transform_single(test_transaction)
        ...
        for key in features_1:
            if key in features_2:
                assert (
                    features_1[key] == features_2[key]
                ), f"Feature {key} differs between cached and non-cached"
    ...
    def test_data_consistency_across_components(self, transaction_generator):
        ...
        for i, features in enumerate(features_list[1:], 1):
            for key in first_features:
                if key in features:
                    assert (
                        first_features[key] == features[key]
                    ), f"Feature {key} inconsistent at iteration {i}"
        ...
        for i, prediction in enumerate(predictions_list[1:], 1):
            assert (
                abs(prediction - first_prediction) < 1e-6
            ), f"Prediction inconsistent at iteration {i}"
    ...
    def test_scalability_integration(self, transaction_generator):
        ...
        for size in data_sizes:
            ...
            print(
                f"      ⏱️ Average time per transaction: {avg_time_per_transaction:.2f}ms"
            )
            print(f"      🚀 Throughput: {throughput:.1f} TPS")

            # Verify acceptable performance
            assert (
                avg_time_per_transaction <= 200
            ), f"Performance degraded at size {size}"
        ...
        for metrics in performance_metrics:
            print(
                f"      {metrics['size']} txn: {metrics['avg_time_ms']:.2f}ms avg, {metrics['throughput_tps']:.1f} TPS"
            )
...
class TestComponentInteractions:
    ...
    def test_feature_processor_ensemble_integration(self, sample_single_transaction):
        ...
        # Process features
        features = feature_processor.transform_single(sample_single_transaction)
