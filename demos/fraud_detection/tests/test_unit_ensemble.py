...
class TestEnsembleFraudDetector:
    ...
    @patch('demos.fraud_detection.models.ensemble_fraud_detector.EnsembleFraudDetector._train_sub_models')
    def test_train_basic(self, mock_train_sub_models, ensemble_detector, sample_features):
    ...
    def test_predict_with_trained_model(self, mock_trained_ensemble, sample_features):
        ...
        mock_trained_ensemble.rule_based_detector.predict = Mock(return_value=np.array([0.2, 0.8, 0.1, 0.9, 0.05]))
        mock_trained_ensemble.anomaly_detector.predict = Mock(return_value=np.array([0.3, 0.7, 0.2, 0.8, 0.1]))
        mock_trained_ensemble.neural_detector.predict = Mock(return_value=np.array([0.1, 0.9, 0.15, 0.85, 0.08]))
        mock_trained_ensemble.behavioral_detector.predict = Mock(return_value=np.array([0.25, 0.75, 0.18, 0.82, 0.12]))
    ...
    def test_edge_cases(self, mock_trained_ensemble):
        ...
        mock_trained_ensemble.rule_based_detector.predict = Mock(return_value=np.array([0.3]))
        mock_trained_ensemble.anomaly_detector.predict = Mock(return_value=np.array([0.4]))
        mock_trained_ensemble.neural_detector.predict = Mock(return_value=np.array([0.2]))
        mock_trained_ensemble.behavioral_detector.predict = Mock(return_value=np.array([0.35]))
    ...
    def test_performance_tracking(self, mock_trained_ensemble, sample_features):
        ...
        # Mock sub-model predictions
        mock_trained_ensemble.rule_based_detector.predict = Mock(return_value=np.array([0.2, 0.8, 0.1]))
        mock_trained_ensemble.anomaly_detector.predict = Mock(return_value=np.array([0.3, 0.7, 0.2]))
        mock_trained_ensemble.neural_detector.predict = Mock(return_value=np.array([0.1, 0.9, 0.15]))
        mock_trained_ensemble.behavioral_detector.predict = Mock(return_value=np.array([0.25, 0.75, 0.18]))
