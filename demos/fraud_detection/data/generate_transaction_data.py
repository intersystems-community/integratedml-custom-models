...
class TransactionDataGenerator:
    ...
    def _initialize_customers(self) -> None:
        ...
        for customer_id in range(self.n_customers):
            customer_type = np.random.choice(
                customer_types, p=[0.3, 0.4, 0.2, 0.05, 0.05]
            )
    ...
    def generate_transactions(
        ...
        logger.info(
            f"Generating {n_legitimate_transactions} legitimate transactions..."
        )
        ...
        logger.info(
            f"Generated {len(df)} total transactions ({df['is_fraud'].sum()} fraudulent)"
        )

        return df

    def _generate_legitimate_transactions(
        self, n_transactions: int, start_date: datetime, end_date: datetime
    ) -> List[Dict]:
    ...
    def _generate_fraud_transaction(
        self, customer: Dict, fraud_type: str, start_date: datetime, end_date: datetime
    ) -> Dict:
        ...
        if fraud_type == "account_takeover":
            return self._generate_account_takeover_transaction(
                customer, start_date, end_date
            )
        elif fraud_type == "card_testing":
            return self._generate_card_testing_transaction(
                customer, start_date, end_date
            )
        elif fraud_type == "synthetic_identity":
            return self._generate_synthetic_identity_transaction(
                customer, start_date, end_date
            )
        elif fraud_type == "velocity_fraud":
            return self._generate_velocity_fraud_transaction(
                customer, start_date, end_date
            )
        elif fraud_type == "manual_entry_fraud":
            return self._generate_manual_entry_fraud_transaction(
                customer, start_date, end_date
            )
        else:
    ...
    def _generate_account_takeover_transaction(
        self, customer: Dict, start_date: datetime, end_date: datetime
    ) -> Dict:
    ...
    def _generate_card_testing_transaction(
        self, customer: Dict, start_date: datetime, end_date: datetime
    ) -> Dict:
    ...
    def _generate_synthetic_identity_transaction(
        self, customer: Dict, start_date: datetime, end_date: datetime
    ) -> Dict:
    ...
    def _generate_velocity_fraud_transaction(
        self, customer: Dict, start_date: datetime, end_date: datetime
    ) -> Dict:
    ...
    def _generate_manual_entry_fraud_transaction(
        self, customer: Dict, start_date: datetime, end_date: datetime
    ) -> Dict:
    ...
    def _generate_realistic_amount(self, customer: Dict, category: str) -> float:
        ...
        base_amount = np.random.normal(
            merchant_info["avg_amount"], merchant_info["amount_std"]
        )
    ...
    def _generate_location_near_home(
        self, customer: Dict, max_distance: float = 50.0
    ) -> Tuple[float, float]:
    ...
    def _add_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["days_since_first"] = (
            df["timestamp"] - df["customer_first_transaction"]
        ).dt.days + 1
        df["transaction_velocity"] = (
            df["customer_transaction_count"] / df["days_since_first"]
        )

        return df

    def generate_streaming_data(
        self, transactions_per_minute: int = 100, duration_minutes: int = 60
    ) -> pd.DataFrame:
        """
        ...
        df = self.generate_transactions(
            n_transactions=total_transactions, start_date=start_time, end_date=end_time
        )
...
def create_sample_dataset(
    output_path: str = None, n_transactions: int = 50000, fraud_rate: float = 0.02
) -> pd.DataFrame:
    """
    ...
    generator = RealisticTransactionGenerator(
        n_customers=5000, fraud_rate=fraud_rate, random_state=42
    )
