...
class LatencyVerificationSystem:
    ...
    def measure_single_transaction_latency(self, iterations: int = 1000) -> Dict[str, float]:
        ...
        transaction_features = pd.DataFrame([{
            ...
            'merchant_risk_score': test_transaction.get('merchant_risk_score', 0.3),
        ...
        for i in range(iterations):
            ...
            if (i + 1) % 100 == 0:
                ...
                print(f"   📊 Progress: {i+1}/{iterations} | Avg so far: {avg_so_far:.2f}ms")
        ...
        results = {
            ...
            'success_rate': (np.array(latencies) <= self.target_latency_ms).mean() * 100
        }
        
        self.results['single_transaction'] = results
        return results
    
    def measure_batch_processing_latency(self, batch_sizes: List[int] = [1, 5, 10, 25, 50, 100]) -> Dict[int, Dict[str, float]]:
        ...
        for batch_size in batch_sizes:
            ...
            batch_latencies = []
            iterations = max(10, 100 // batch_size)  # Fewer iterations for larger batches
            ...
            batch_results[batch_size] = {
                ...
                'success_rate': (np.array(batch_latencies) <= self.target_latency_ms).mean() * 100
            }
            
            print(f"      Avg per transaction: {batch_results[batch_size]['mean_per_transaction']:.2f}ms")
            print(f"      Throughput: {batch_results[batch_size]['throughput_tps']:.1f} TPS")
    ...
    def measure_concurrent_processing_latency(self) -> Dict[str, float]:
        ...
        for num_threads in [1, 2, 4, 8]:
            ...
            print(f"      Avg latency: {concurrency_results[num_threads]['mean']:.2f}ms")
        
        self.results['concurrent_processing'] = concurrency_results
        return concurrency_results
    
    def run_comprehensive_verification(self, iterations: int = 1000, batch_sizes: List[int] = [1, 5, 10, 25, 50]) -> Dict[str, Any]:
    ...
    def _generate_verification_report(self, total_time: float):
        ...
        print(f"   Min/Max Latency:     {single.get('min', 0):.2f}ms / {single.get('max', 0):.2f}ms")
        print(f"   Success Rate:        {single.get('success_rate', 0):.1f}% (≤ {self.target_latency_ms}ms)")
        ...
        if avg_latency <= self.target_latency_ms:
            print(f"   ✅ Average Latency: PASS ({avg_latency:.2f}ms ≤ {self.target_latency_ms}ms)")
        else:
            print(f"   ❌ Average Latency: FAIL ({avg_latency:.2f}ms > {self.target_latency_ms}ms)")
        
        if p95_latency <= self.target_latency_ms * 1.5:
            print(f"   ✅ P95 Latency: PASS ({p95_latency:.2f}ms ≤ {self.target_latency_ms * 1.5}ms)")
        else:
            print(f"   ❌ P95 Latency: FAIL ({p95_latency:.2f}ms > {self.target_latency_ms * 1.5}ms)")
        ...
        if batch:
            ...
            for batch_size, metrics in batch.items():
                print(f"   Batch {batch_size:3d}: {metrics['mean_per_transaction']:.2f}ms avg, {metrics['throughput_tps']:.1f} TPS")
        ...
        if concurrent:
            ...
            for threads, metrics in concurrent.items():
                print(f"   {threads} threads: {metrics['mean']:.2f}ms avg, {metrics['success_rate']:.1f}% success")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Verify fraud detection latency requirements")
    parser.add_argument("--iterations", type=int, default=1000, help="Number of iterations for single transaction test")
    parser.add_argument("--batch-sizes", type=str, default="1,5,10,25,50", help="Comma-separated batch sizes to test")
