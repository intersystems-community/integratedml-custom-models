...
class ModelOptimizer:
    ...
    def create_fast_path_rules(self, training_data: pd.DataFrame, 
        ...
        fraud_rate_high_amount = labels[training_data['amount'] > high_amount_threshold].mean()
        ...
        if 'impossible_travel' in training_data.columns:
            impossible_travel_fraud_rate = labels[training_data['impossible_travel'] == 1].mean()
        ...
        if 'customer_rapid_fire_1m' in training_data.columns:
            rapid_fire_fraud_rate = labels[training_data['customer_rapid_fire_1m'] == 1].mean()
        ...
        if 'ensemble_risk_score' in training_data.columns:
            ...
            low_risk_fraud_rate = labels[training_data['ensemble_risk_score'] <= low_risk_threshold].mean()
            
            if low_risk_fraud_rate < 0.01:
                self.fast_path_rules.append({
                    'condition': lambda x: x.get('ensemble_risk_score', 0.5) <= low_risk_threshold,
    ...
    def predict_optimized(self, model, features: Union[Dict[str, Any], pd.DataFrame],
    ...
    def _generate_cache_key(self, features: Dict[str, Any], 
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        if total_predictions > 0:
            ...
            stats['fast_path_rate'] = (stats['fast_path_hits'] / total_predictions) * 100
        ...
        stats['cache_utilization'] = (len(self.prediction_cache.cache) / self.cache_size) * 100
...
class ModelSelector:
    ...
    def update_performance(self, model_name: str, prediction_time: float, 
                         accuracy_estimate: Optional[float] = None) -> None:
        """
        ...
        for model_name, perf in self.model_performance.items():
            ...
            if perf['prediction_times']:
                model_summary.update({
                    ...
                    'median_prediction_time_ms': np.median(perf['prediction_times']),
                    'p95_prediction_time_ms': np.percentile(perf['prediction_times'], 95)
                })
