...
class CreditDataPreprocessor:
    ...
    def validate_data(
            ...
            if spec["type"] == "numeric":
                if not pd.api.types.is_numeric_dtype(X[feature]):
                    issues["invalid_types"].append(
                        f"{feature}: expected numeric, got {X[feature].dtype}"
                    )
                else:
                    # Check range
                    if "min" in spec and X[feature].min() < spec["min"]:
                        issues["out_of_range"].append(
                            f"{feature}: values below minimum {spec['min']}"
                        )
                    if "max" in spec and X[feature].max() > spec["max"]:
                        issues["out_of_range"].append(
                            f"{feature}: values above maximum {spec['max']}"
                        )

            # Check categorical features
            elif spec["type"] == "categorical":
                if "values" in spec:
                    invalid_values = set(X[feature].dropna().unique()) - set(
                        spec["values"]
                    )
                    if invalid_values:
            ...
            if missing_count > 0:
                ...
                if missing_pct > 50:
                    issues["warnings"].append(
                        f"{feature}: {missing_pct:.1f}% missing values"
                    )
        ...
        problematic_features.extend(
            [feat.split(":")[0] for feat in issues["invalid_types"]]
        )
        problematic_features.extend(
            [feat.split(":")[0] for feat in issues["out_of_range"]]
        )
        problematic_features.extend(
            [feat.split(":")[0] for feat in issues["invalid_categories"]]
        )
        issues["problematic_features"] = list(
            set(problematic_features)
        )  # Remove duplicates

        # Determine if there are critical issues
        critical_issues = (
            len(issues["missing_features"]) > 0
            or len(issues["invalid_types"]) > 0
            or len(issues["out_of_range"]) > 0
            or total_missing > len(X) * 0.5  # More than 50% missing data
        )
    ...
    def clean_data(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        if "employment_duration" in X_clean.columns:
            X_clean["employment_duration"] = np.maximum(
                X_clean["employment_duration"], 0
            )
    ...
    def fit_transform(
        ...
        if y is not None:
            ...
            y_clean = (
                y.loc[remaining_indices]
                if len(remaining_indices) < len(original_indices)
                else y
            )
        else:
            X_clean = self.clean_data(X)
        ...
        for feature in self.categorical_features:
            le = LabelEncoder()
            X_transformed[feature] = le.fit_transform(
    ...
    def _identify_feature_types(self, X: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        logger.debug(
            f"Identified {len(self.numerical_features)} numerical, "
    ...
    def _apply_transformations(
        ...
        for feature in self.categorical_features:
            if feature in self.label_encoders:
                ...
                try:
                    X_transformed[feature] = le.transform(
                        X_transformed[feature].astype(str)
                    )
                except ValueError:
...
def load_and_preprocess_data(
    ...
    if target_column in data.columns:
        ...
        y = None
        logger.warning(
            f"Target column '{target_column}' not found. Proceeding without target."
        )
    ...
    if y is not None:
        ...
        logger.info(
            f"Training set: {X_train.shape}, Default rate: {y_train.mean():.2%}"
        )
...
def create_feature_summary(
    ...
    for feature in X.columns:
        ...
        if pd.api.types.is_numeric_dtype(X[feature]):
            ...
            # Categorical feature
            mode_value = (
                X[feature].mode().iloc[0] if not X[feature].mode().empty else None
            )
            feature_info.update(
                {
                    "mode": mode_value,
                    "mode_frequency": (
                        (X[feature] == mode_value).sum() if mode_value else 0
                    ),
                }
            )
    ...
    if y is not None and "target_correlation" in summary_df.columns:
        summary_df = summary_df.sort_values(
            "target_correlation", key=abs, ascending=False
        )
...
def main():
    ...
    try:
        ...
        print(
            "Could not import data generator. Please ensure the generate_sample_data.py is available."
        )
    except Exception as e:
