...
from demos.credit_risk.tests.test_credit_risk_classifier import (
    TestCustomCreditRiskClassifier, TestUtilityFunctions
)
from demos.credit_risk.tests.test_data_preprocessing import (
    TestCreditDataPreprocessor, TestPreprocessingUtilities
)
from demos.credit_risk.tests.test_integration import (
    TestEndToEndWorkflow, TestDataPipelineIntegration, TestErrorHandlingIntegration
)
...
def create_test_suite(test_type='all'):
    ...
    if test_type in ['all', 'unit']:
        # Unit tests
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestCustomCreditRiskClassifier))
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestUtilityFunctions))
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestCreditDataPreprocessor))
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestPreprocessingUtilities))
    
    if test_type in ['all', 'integration']:
        # Integration tests
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestEndToEndWorkflow))
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestDataPipelineIntegration))
        suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestErrorHandlingIntegration))
...
def run_with_coverage(suite, verbosity=1):
    ...
    try:
        ...
        runner = unittest.TextTestRunner(
            verbosity=verbosity,
            resultclass=create_result_class(results)
        )
...
def run_performance_benchmarks():
    ...
    try:
        ...
        configurations = [
            ('Baseline', {'enable_debt_ratio': False, 'enable_interaction_terms': False, 'enable_risk_scoring': False}),
            ('Full Features', {'enable_debt_ratio': True, 'enable_interaction_terms': True, 'enable_risk_scoring': True})
        ]
        ...
        for config_name, params in configurations:
            ...
            print(f"     Prediction time: {prediction_time:.3f} seconds ({len(X_test)} samples)")
            print(f"     Predictions/second: {len(X_test)/prediction_time:.0f}")
        
        total_time = generation_time + preprocessing_time + training_time + prediction_time
...
def main():
    ...
    parser.add_argument('-p', '--performance', action='store_true', help='Include performance benchmarks')
