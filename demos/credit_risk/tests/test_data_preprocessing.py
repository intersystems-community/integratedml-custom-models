...
class TestCreditDataPreprocessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.clean_X, self.clean_y = self.generator.generate_dataset(n_samples=100, default_rate=0.3)
        ...
        self.dirty_y = pd.concat([self.dirty_y, self.dirty_y.iloc[0:1]], ignore_index=True)
    ...
    def test_missing_value_handling(self):
        ...
        try:
            X_no_handle, _ = preprocessor_no_handle.fit_transform(X_missing, self.clean_y)
    ...
    def test_outlier_detection_and_removal(self):
        ...
        X_processed, y_processed = preprocessor_remove.fit_transform(X_outliers, self.clean_y)
        ...
        X_no_removal, y_no_removal = preprocessor_keep.fit_transform(X_outliers, self.clean_y)
    ...
    def test_feature_normalization(self):
        ...
        for feature in preprocessor_no_norm.numerical_features:
            if len(X_no_norm[feature].unique()) > 2:  # Skip binary features
                ...
                self.assertAlmostEqual(X_no_norm[feature].std(), original_std, delta=0.1)
    ...
    def test_categorical_encoding(self):
        ...
        if 'categorical_test' not in X_categorical.columns:
            X_categorical['categorical_test'] = np.random.choice(['A', 'B', 'C'], size=len(X_categorical))
    ...
    def test_duplicate_removal(self):
        ...
        X_duplicates = pd.concat([self.clean_X, self.clean_X.iloc[:5]], ignore_index=True)
        y_duplicates = pd.concat([self.clean_y, self.clean_y.iloc[:5]], ignore_index=True)
        
        original_shape = X_duplicates.shape
        
        X_processed, y_processed = self.preprocessor.fit_transform(X_duplicates, y_duplicates)
    ...
    def test_fit_transform_consistency(self):
        ...
        X_fit_transform, y_fit_transform = self.preprocessor.fit_transform(self.clean_X, self.clean_y)
    ...
    def test_preprocessing_pipeline_integrity(self):
        ...
        X_processed, y_processed = self.preprocessor.fit_transform(self.dirty_X, self.dirty_y)
        ...
        # No infinite values
        self.assertFalse(np.isinf(X_processed.select_dtypes(include=[np.number])).any().any())
    ...
    def test_save_load_preprocessor(self):
        ...
        try:
            ...
            # Load preprocessor
            loaded_preprocessor = CreditDataPreprocessor.load_preprocessor(preprocessor_path)
            ...
            pd.testing.assert_frame_equal(original_transform, loaded_transform, check_dtype=False)
    ...
    def test_edge_cases(self):
        ...
        try:
            X_processed, _ = self.preprocessor.fit_transform(X_all_missing, self.clean_y)
