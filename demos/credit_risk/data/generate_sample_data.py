...
class CreditDataGenerator:
    ...
    def _reset_random_state(self):
        ...
        """
        logger.debug(f"Resetting random seed to {self.random_seed} for reproducible generation")
    ...
    def generate_dataset(self,
                        n_samples: int = 1000,
                        default_rate: float = 0.3) -> Tuple[pd.DataFrame, pd.Series]:
        """
    ...
    def _generate_employment_data(self, n_samples: int) -> dict:
        ...
        employment_data['job'] = np.random.choice(job_categories, n_samples, p=job_weights)
    ...
    def _generate_housing_data(self, n_samples: int) -> dict:
        ...
        housing_data['housing'] = np.random.choice(housing_types, n_samples, p=housing_weights)
    ...
    def _generate_credit_request_data(self, n_samples: int) -> dict:
        ...
        credit_data['purpose'] = np.random.choice(purposes, n_samples, p=purpose_weights)
    ...
    def _generate_financial_data(self, n_samples: int, credit_amounts: np.ndarray) -> dict:
    ...
    def _generate_credit_history_data(self, n_samples: int) -> dict:
        ...
        history_weights = [info['weight'] for info in self.credit_history_types.values()]
    ...
    def _generate_target_variable(self, X: pd.DataFrame, target_default_rate: float) -> pd.Series:
        """
        ...
        # Purpose risk
        purpose_risk_map = {
            'business': 0.4, 'vacation': 0.3, 'others': 0.2,
            'domestic': 0.1, 'repairs': 0.1, 'education': 0.05,
            'car': 0.05, 'furniture': 0.05, 'radio/tv': 0.05
        }
...
def main():
    ...
    pred_path = generator.save_dataset(X_pred, None, 'credit_risk_examples.csv', 
