...
class ForecastEvaluator:
    ...
    def __init__(self,
        ...
        self.business_metrics = business_metrics or [
            'revenue_impact', 'inventory_optimization', 'stockout_cost', 
            'holding_cost', 'forecast_value_added'
        ]
    ...
    def evaluate_forecast(self, 
                         ...
                         external_factors: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """
        ...
        print(f"Evaluation complete. Overall MAPE: {self.accuracy_metrics_['mape']:.2f}%")
        
        return results
    
    def analyze_business_scenarios(self, 
                                 forecast: pd.Series,
    ...
    def calculate_forecast_value_added(self, 
                                     forecast: pd.Series,
        ...
        """
        # Align data
        common_index = forecast.index.intersection(naive_forecast.index).intersection(actual.index)
        forecast_aligned = forecast.loc[common_index]
        ...
        fva_median = (naive_errors.median() - model_errors.median()) / naive_errors.median() * 100
    ...
    def generate_business_report(self) -> str:
        ...
        report_sections.append(f"Mean Absolute Percentage Error: {self.accuracy_metrics_['mape']:.2f}%")
        report_sections.append(f"Business Impact Score: {summary['business_impact_score']:.2f}/10")
        report_sections.append("")
        ...
        report_sections.append(f"Mean Absolute Error (MAE): {self.accuracy_metrics_['mae']:.2f}")
        report_sections.append(f"Root Mean Square Error (RMSE): {self.accuracy_metrics_['rmse']:.2f}")
        report_sections.append(f"Mean Absolute Percentage Error (MAPE): {self.accuracy_metrics_['mape']:.2f}%")
        report_sections.append(f"Forecast Bias: {self.accuracy_metrics_['bias']:.2f}")
        report_sections.append(f"Tracking Signal: {self.accuracy_metrics_['tracking_signal']:.2f}")
        report_sections.append("")
        ...
        if 'revenue_impact' in self.business_insights_:
            ...
            report_sections.append(f"Estimated Revenue Impact: ${revenue_impact['total_impact']:,.2f}")
            report_sections.append(f"Revenue at Risk: ${revenue_impact['revenue_at_risk']:,.2f}")
        
        if 'inventory_optimization' in self.business_insights_:
            inventory = self.business_insights_['inventory_optimization']
            report_sections.append(f"Inventory Optimization Savings: ${inventory['potential_savings']:,.2f}")
            report_sections.append(f"Stockout Risk Reduction: {inventory['stockout_reduction']:.1f}%")
    ...
    def plot_evaluation_dashboard(self, 
                                actual: pd.Series,
                                forecast: pd.Series,
        ...
        fig.suptitle('Sales Forecast Evaluation Dashboard', fontsize=16, fontweight='bold')
        ...
        if 'revenue_impact' in self.business_insights_:
            ...
            values = [
                ...
                self.business_insights_.get('inventory_optimization', {}).get('potential_savings', 0),
    ...
    def _calculate_accuracy_metrics(self, 
                                  actual: pd.Series,
                                  forecast: pd.Series,
                                  forecast_intervals: Optional[Dict] = None) -> Dict[str, float]:
        ...
        # R-squared
        r_squared = 1 - (np.sum(squared_errors) / np.sum((actual_clean - actual_clean.mean()) ** 2))
    ...
    def _calculate_business_insights(self, 
                                   actual: pd.Series,
                                   forecast: pd.Series,
                                   external_factors: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    ...
    def _analyze_performance_patterns(self, 
                                    actual: pd.Series,
                                    forecast: pd.Series) -> Dict[str, Any]:
    ...
    def _calculate_revenue_impact(self, actual: pd.Series, forecast: pd.Series) -> Dict[str, float]:
        """Calculate revenue impact of forecast accuracy."""
        errors = forecast - actual  # Positive = over-forecast, Negative = under-forecast
        ...
        holding_cost_total = over_forecast * self.cost_parameters['holding_cost_per_unit']
        
        # Under-forecasting impact (stockout costs)
        under_forecast = abs(errors[errors < 0].sum())
        stockout_cost_total = under_forecast * self.cost_parameters['stockout_cost_per_unit']
        ...
        # Opportunity cost
        opportunity_cost = over_forecast * revenue_per_unit * 0.1  # 10% opportunity cost
    ...
    def _calculate_inventory_optimization(self, actual: pd.Series, forecast: pd.Series) -> Dict[str, float]:
        ...
        if actual_std > 0:
            ...
            safety_stock_reduction = max(0, accuracy_improvement * 0.5)  # Conservative estimate
        else:
        ...
        safety_stock_savings = (avg_forecast * safety_stock_reduction * 
        ...
        stockout_reduction = max(0, (baseline_stockout_rate - stockout_rate) / baseline_stockout_rate * 100)
        ...
        return {
            ...
            'inventory_turnover_improvement': accuracy_improvement * 100 if 'accuracy_improvement' in locals() else 0
        }
    
    def _assess_forecast_reliability(self, actual: pd.Series, forecast: pd.Series) -> Dict[str, float]:
        ...
        error_cv = error_std / abs_errors.mean() if abs_errors.mean() > 0 else float('inf')
        ...
        forecast_stability = 1 - (forecast_changes.std() / forecast.mean()) if forecast.mean() > 0 else 0
    ...
    def _analyze_seasonal_performance(self, actual: pd.Series, forecast: pd.Series) -> Dict[str, Any]:
                ...
                for i in range(period):
                    ...
                    if mask.sum() > 0:
                        ...
                        if len(actual_seasonal) > 0:
                            errors = np.abs((actual_seasonal - forecast_seasonal) / actual_seasonal) * 100
                            seasonal_groups[i] = {
                                'mape': errors.mean(),
    ...
    def _calculate_business_risk(self, actual: pd.Series, forecast: pd.Series) -> Dict[str, float]:
        ...
        # Volatility risk
        forecast_volatility = forecast.std() / forecast.mean() if forecast.mean() > 0 else 0
        ...
        return {
            ...
            'risk_score': min(10, tail_risk + volatility_risk * 10)  # Combined risk score
        }
    ...
    def _calculate_interval_metrics(self, actual: pd.Series, intervals: Dict) -> Dict[str, float]:
    ...
    def _evaluate_scenario(self, forecast: pd.Series, scenario_params: Dict[str, float]) -> Dict[str, float]:
    ...
    def _generate_actionable_insights(self) -> List[str]:
        ...
        if mape < 5:
            insights.append("Excellent forecast accuracy achieved. Consider using this model for critical business decisions.")
        elif mape < 10:
            insights.append("Good forecast accuracy. Model is suitable for operational planning.")
        elif mape < 20:
            insights.append("Moderate forecast accuracy. Consider improving feature engineering or model tuning.")
        else:
            insights.append("Poor forecast accuracy. Recommend model revision or additional data sources.")
        
        # Bias insights
        bias = self.accuracy_metrics_['bias']
        if abs(bias) > self.accuracy_metrics_['mae'] * 0.5:
            if bias > 0:
                insights.append("Forecast shows significant positive bias (over-forecasting). Review demand assumptions.")
            else:
                insights.append("Forecast shows significant negative bias (under-forecasting). May lead to stockouts.")
        
        # Business impact insights
        if 'revenue_impact' in self.business_insights_:
            revenue_impact = self.business_insights_['revenue_impact']['total_impact']
            if revenue_impact < -1000:
                insights.append(f"Forecast errors creating significant business impact (${abs(revenue_impact):,.0f}). Prioritize accuracy improvements.")
        
        # Seasonal performance insights
        if 'seasonal_performance' in self.business_insights_:
            insights.append("Monitor seasonal performance patterns to identify improvement opportunities.")
        
        # Risk insights
        if 'risk_assessment' in self.business_insights_:
            risk_score = self.business_insights_['risk_assessment']['risk_score']
            if risk_score > 7:
                insights.append("High forecast risk detected. Implement robust contingency planning.")
    ...
    def _create_evaluation_summary(self) -> Dict[str, Any]:
        ...
        revenue_impact = self.business_insights_.get('revenue_impact', {}).get('total_impact', 0)
        reliability_score = self.business_insights_.get('forecast_reliability', {}).get('reliability_score', 5)
        
        business_impact_score = min(10, max(0, reliability_score - abs(revenue_impact) / 10000))
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if mape > 15:
            recommendations.append("Improve model accuracy by adding more relevant features or trying different algorithms")
        
        # Bias recommendations
        if abs(bias) > self.accuracy_metrics_['mae'] * 0.3:
            recommendations.append("Address forecast bias through calibration or model adjustment")
        
        # Business recommendations
        if 'revenue_impact' in self.business_insights_:
            revenue_impact = self.business_insights_['revenue_impact']['total_impact']
            if revenue_impact < -5000:
                recommendations.append("Implement forecast accuracy improvement initiative due to significant business impact")
        
        # Risk recommendations
        if 'risk_assessment' in self.business_insights_:
            tail_risk = self.business_insights_['risk_assessment']['tail_risk_percentage']
            if tail_risk > 10:
                recommendations.append("Develop contingency plans for extreme demand scenarios")
