...
class BusinessIntelligence:
    ...
    def __init__(self,
        ...
        self.market_segments = market_segments or [
            'Premium', 'Standard', 'Budget', 'Enterprise', 'Consumer'
        ]
    ...
    def generate_executive_dashboard(self, 
        ...
        dashboard = {
            ...
            'executive_summary': self._create_executive_summary(core_kpis, health_metrics)
        }
    ...
    def analyze_market_performance(self, 
        ...
        if competitor_data is not None:
            competitive_position = self._analyze_competitive_position(sales_data, competitor_data)
    ...
    def calculate_roi_metrics(self, 
                            sales_data: pd.Series,
                            forecast_data: pd.Series,
                            investment_costs: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        payback_period = total_costs / (total_benefits / 12) if total_benefits > 0 else float('inf')  # months
        
        # NPV calculation (simplified, 5-year horizon, 10% discount rate)
        annual_benefits = total_benefits
        discount_rate = 0.10
        npv = sum([annual_benefits / (1 + discount_rate)**year for year in range(1, 6)]) - total_costs
    ...
    def generate_periodic_reports(self, 
        ...
        if period not in self.reporting_periods:
            raise ValueError(f"Period {period} not supported. Use one of {self.reporting_periods}")
        ...
        cv_sales = grouped_sales.std() / grouped_sales.mean() if grouped_sales.mean() > 0 else 0
        ...
        report = {
            ...
            'recommendations': self._generate_period_recommendations(grouped_sales, period)
        }
        
        return report
    
    def create_kpi_scorecard(self, 
    ...
    def export_dashboard_data(self, format_type: str = 'json') -> str:
        ...
        if not self.is_analyzed_:
            raise ValueError("No dashboard data available. Run generate_executive_dashboard() first.")
    ...
    def _calculate_core_kpis(self, sales_data: pd.Series, forecast_data: pd.Series) -> Dict[str, float]:
        ...
        if len(common_index) > 0:
            ...
            forecast_accuracy = 100 - np.mean(np.abs((actual_subset - forecast_subset) / actual_subset)) * 100
        else:
            forecast_accuracy = 0
        ...
        correlation = np.corrcoef(x, sales_data.values)[0, 1] if len(sales_data) > 1 else 0
    ...
    def _analyze_performance_trends(self, sales_data: pd.Series, forecast_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(sales_data) > 10:
            ...
            momentum = ((recent_trend - older_trend) / older_trend * 100) if older_trend > 0 else 0
            trends['momentum'] = momentum
        
        # Seasonality strength
        if len(sales_data) > 14:
            weekly_pattern = [sales_data[sales_data.index.dayofweek == i].mean() for i in range(7)]
            seasonality_strength = np.std(weekly_pattern) / np.mean(weekly_pattern) if np.mean(weekly_pattern) > 0 else 0
            trends['weekly_seasonality'] = seasonality_strength
        
        return trends
    
    def _calculate_business_health(self, sales_data: pd.Series, forecast_data: pd.Series) -> Dict[str, float]:
        ...
        consistency = 1 - (rolling_mean.std() / rolling_mean.mean()) if rolling_mean.mean() > 0 else 0
        performance_consistency = max(0, min(10, consistency * 10))
        health['performance_consistency'] = performance_consistency
        
        # Overall health score
        overall_health = (stability_score + growth_sustainability + performance_consistency) / 3
    ...
    def _generate_strategic_recommendations(self, 
        ...
        if growth_rate < 5:
            recommendations.append("Consider growth initiatives: market expansion, new products, or customer acquisition")
        elif growth_rate > 20:
            recommendations.append("High growth detected: ensure operational capacity can support continued expansion")
        
        # Volatility recommendations
        volatility = kpis.get('sales_volatility', 0)
        if volatility > 0.3:
            recommendations.append("High sales volatility: implement demand smoothing strategies or diversification")
        
        # Forecast accuracy recommendations
        accuracy = kpis.get('forecast_accuracy', 0)
        if accuracy < 80:
            recommendations.append("Improve forecasting accuracy through better data collection and model enhancement")
        
        # Trend recommendations
        trend_strength = kpis.get('trend_strength', 0)
        if trend_strength < 0.3:
            recommendations.append("Weak trend signal: focus on identifying and strengthening growth drivers")
        ...
        if hasattr(sales_data.index, 'dayofweek'):
            ...
            if dow_variation / dow_mean > 0.2:
                recommendations.append("Strong day-of-week patterns: optimize staffing and inventory for peak days")
        
        return recommendations
    
    def _assess_business_risks(self, sales_data: pd.Series, forecast_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(sales_data) > 20:
            ...
            trend_divergence = abs(recent_trend - overall_trend) / abs(overall_trend) if overall_trend != 0 else 0
            
            trend_risk = 'High' if trend_divergence > 0.5 else 'Medium' if trend_divergence > 0.2 else 'Low'
            risks['trend_reversal_risk'] = trend_risk
        
        # Forecast reliability risk
        common_index = sales_data.index.intersection(forecast_data.index)
        if len(common_index) > 0:
            forecast_errors = np.abs(sales_data.loc[common_index] - forecast_data.loc[common_index])
            error_cv = forecast_errors.std() / forecast_errors.mean() if forecast_errors.mean() > 0 else 0
            
            forecast_risk = 'High' if error_cv > 0.5 else 'Medium' if error_cv > 0.25 else 'Low'
        ...
        if len(sales_data) > 30:
            ...
            if len(monthly_sales) > 1:
                ...
                seasonal_risk = 'High' if seasonal_cv > 0.4 else 'Medium' if seasonal_cv > 0.2 else 'Low'
                risks['seasonal_dependency_risk'] = seasonal_risk
        
        return risks
    
    def _identify_opportunities(self, sales_data: pd.Series, forecast_data: pd.Series) -> List[Dict[str, Any]]:
        ...
        common_index = sales_data.index.intersection(forecast_data.index)
        if len(common_index) > 0:
            accuracy = 100 - np.mean(np.abs((sales_data.loc[common_index] - forecast_data.loc[common_index]) / sales_data.loc[common_index])) * 100
    ...
    def _create_executive_summary(self, kpis: Dict[str, float], health: Dict[str, float]) -> str:
        ...
        summary_parts.append(f"Total revenue: ${revenue:,.0f} with {growth:+.1f}% growth rate")
        ...
        summary_parts.append(f"Business health: {health_status} ({health_score:.1f}/10)")
        
        return ". ".join(summary_parts) + "."
    
    def _analyze_seasonal_patterns(self, data: pd.Series, period: str) -> Dict[str, Any]:
        ...
        if period == 'monthly' and hasattr(data.index, 'month'):
            ...
            analysis['seasonal_variation'] = (monthly_avg.max() - monthly_avg.min()) / monthly_avg.mean()
            ...
            analysis['seasonal_variation'] = (weekly_avg.max() - weekly_avg.min()) / weekly_avg.mean()
    ...
    def _generate_period_recommendations(self, data: pd.Series, period: str) -> List[str]:
        ...
        if cv > 0.3:
            recommendations.append(f"High {period} variability detected - investigate causes and implement stabilization measures")
        
        # Growth trend
        if len(data) > 2:
            growth_rate = (data.iloc[-1] - data.iloc[0]) / data.iloc[0] * 100 if data.iloc[0] > 0 else 0
            if growth_rate < -5:
                recommendations.append(f"Declining {period} trend - urgent action needed to reverse negative trajectory")
            elif growth_rate > 20:
                recommendations.append(f"Strong {period} growth - ensure capacity planning to support continued expansion")
    ...
    def _calculate_market_share(self, sales_data: pd.Series, market_data: pd.DataFrame) -> Dict[str, float]:
        """Calculate market share metrics."""
        # This is a simplified example - would need actual market data
        total_market = market_data.sum().sum() if not market_data.empty else sales_data.sum() * 10
    ...
    def _analyze_competitive_position(self, sales_data: pd.Series, competitor_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        competitive_position = {
            ...
            'market_position': 'Strong' if our_growth > 10 else 'Average' if our_growth > 0 else 'Weak',
            'competitive_advantage': 'Growth rate' if our_growth > 10 else 'Stability' if our_growth > 0 else 'None identified'
        }
    ...
    def _analyze_market_trends(self, sales_data: pd.Series) -> Dict[str, Any]:
        ...
        if len(sales_data) > 30:
            ...
            trend_analysis = {
                ...
                'market_momentum': 'Strong' if recent_trend > overall_trend * 1.2 else 'Moderate' if recent_trend > overall_trend else 'Weak'
            }
        else:
    ...
    def _identify_growth_opportunities(self, sales_data: pd.Series) -> List[Dict[str, str]]:
...
def main():
    ...
    print(f"\nKPI Scorecard:")
    print(f"  Overall Rating: {scorecard['overall']['rating']} ({scorecard['overall']['score']:.0f}%)")
    
    for kpi, details in scorecard.items():
        if kpi != 'overall':
            print(f"  {kpi.replace('_', ' ').title()}: {details['rating']} "
