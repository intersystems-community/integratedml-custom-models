...
class SalesDataGenerator:
    ...
    def __init__(self,
        ...
        if categories is not None:
            ...
            self.product_categories = [
                ...
                'Health_Beauty', 'Books_Media', 'Food_Beverages', 'Toys_Games'
            ]
    ...
    def _initialize_category_characteristics(self) -> None:
        ...
        category_patterns = {
            'Electronics': {'seasonal_peaks': [11, 12], 'growth_rate': 0.05, 'volatility': 0.15},
            'Clothing': {'seasonal_peaks': [3, 6, 9, 11], 'growth_rate': 0.02, 'volatility': 0.20},
            'Home_Garden': {'seasonal_peaks': [4, 5, 6], 'growth_rate': 0.03, 'volatility': 0.12},
            'Sports_Outdoors': {'seasonal_peaks': [5, 6, 7, 8], 'growth_rate': 0.04, 'volatility': 0.18},
            'Health_Beauty': {'seasonal_peaks': [1, 6], 'growth_rate': 0.06, 'volatility': 0.10},
            'Books_Media': {'seasonal_peaks': [9, 12], 'growth_rate': -0.02, 'volatility': 0.25},
            'Food_Beverages': {'seasonal_peaks': [7, 11, 12], 'growth_rate': 0.01, 'volatility': 0.08},
            'Toys_Games': {'seasonal_peaks': [11, 12], 'growth_rate': 0.03, 'volatility': 0.30}
        }
    ...
    def _generate_holidays(self) -> pd.DataFrame:
        ...
        holiday_definitions = {
            ...
            'Easter': {'month': 4, 'day': 15, 'impact': 1.1, 'duration': 2},  # Simplified
            'Memorial Day': {'month': 5, 'day': 25, 'impact': 1.3, 'duration': 3},  # Last Monday
            'Independence Day': {'month': 7, 'day': 4, 'impact': 1.2, 'duration': 1},
            'Labor Day': {'month': 9, 'day': 6, 'impact': 1.1, 'duration': 3},  # First Monday
            'Halloween': {'month': 10, 'day': 31, 'impact': 1.4, 'duration': 1},
            'Thanksgiving': {'month': 11, 'day': 25, 'impact': 0.3, 'duration': 1},  # Stores closed
            'Black Friday': {'month': 11, 'day': 26, 'impact': 2.5, 'duration': 1},
            'Cyber Monday': {'month': 11, 'day': 29, 'impact': 2.0, 'duration': 1},
            'Christmas': {'month': 12, 'day': 25, 'impact': 0.2, 'duration': 1},  # Stores closed
    ...
    def _generate_economic_indicators(self) -> pd.DataFrame:
        ...
        gdp_cycle = np.sin(2 * np.pi * np.arange(n_days) / (4 * 365)) * 0.01  # 4-year cycle
        gdp_shock_2020 = np.where((dates >= '2020-03-01') & (dates <= '2020-06-01'), -0.05, 0)
        ...
        unemployment_shock_2020 = np.where((dates >= '2020-03-01') & (dates <= '2020-08-01'), 3.0, 0)
        unemployment_rate = base_unemployment + unemployment_seasonal + unemployment_shock_2020
        ...
        confidence_shock_2020 = np.where((dates >= '2020-03-01') & (dates <= '2020-12-01'), -20, 0)
                    ...
                    for day in range(details['duration']):
                        ...
                        if promo_date <= self.end_date:
                            promotions_data.append({
                                ...
                                'participating_stores': ','.join(participating_stores),
                                'participating_categories': ','.join(participating_categories)
                            })
    ...
    def generate_sales_data(self) -> pd.DataFrame:
        ...
        print(f"Generating sales data for {len(self.stores)} stores and {len(self.product_categories)} categories...")
        ...
        for date_idx, date in enumerate(self.date_range):
            ...
            for store in self.stores:
                ...
                for category in self.product_categories:
                    ...
                    base_sales = store_chars['base_sales'] * np.random.uniform(0.1, 0.4)  # Category share
                    
                    # Apply growth trend
                    growth_factor = (1 + category_chars['growth_rate']) ** years_since_start
                    
                    # Seasonal patterns
                    seasonal_factor = self._calculate_seasonal_factor(
                        day_of_year, month, day_of_week, category_chars['seasonal_peaks']
                    )
                    
                    # Store-specific factors
                    store_factor = (
                        store_chars['size_factor'] * 
                        store_chars['economic_factor']
                    )
                    ...
                    noise = np.random.normal(1, category_chars['volatility'] * self.noise_level)
    ...
    def _get_holiday_factor(self, date: pd.Timestamp) -> float:
        ...
        for _, holiday in self.holidays.iterrows():
            ...
            if 0 < days_diff <= holiday['duration_days']:
                ...
                decay = (holiday['duration_days'] - days_diff) / holiday['duration_days']
    ...
    def _get_economic_factor(self, date: pd.Timestamp) -> float:
        ...
        econ_data = self.economic_indicators[self.economic_indicators['date'].dt.date == date.date()]
        ...
        confidence_effect = (econ_row['consumer_confidence'] - 75) / 100  # Normalize around 75
        unemployment_effect = -(econ_row['unemployment_rate'] - 5) / 20  # Normalize around 5%
    ...
    def _get_weather_factor(self, date: pd.Timestamp) -> float:
        ...
        weather_data = self.weather_data[self.weather_data['date'].dt.date == date.date()]
    ...
    def _get_promotion_factor(self, date: pd.Timestamp, store: str, category: str) -> float:
        ...
        date_promotions = self.promotions[self.promotions['date'].dt.date == date.date()]
        ...
        for _, promo in date_promotions.iterrows():
            ...
            if store in participating_stores and category in participating_categories:
                ...
                sales_boost = 1 + (discount_rate * 2)  # 15% discount -> 30% sales increase
                return sales_boost
        
        return 1.0
    
    def _calculate_seasonal_factor(self, day_of_year: int, month: int, 
                                 day_of_week: int, seasonal_peaks: List[int]) -> float:
    ...
    def save_datasets(self, output_dir: str = 'demos/sales_forecasting/data/') -> Dict[str, str]:
        """
        ...
        file_paths = {'main_sales': str(sales_file), 'external_factors': str(external_file)}
    ...
    def _create_external_factors_dataset(self) -> pd.DataFrame:
        ...
        external_data['is_holiday'] = external_data['date'].isin(self.holidays['date']).astype(int)
        
        # Add weather data
        weather_subset = self.weather_data[['date', 'temperature_celsius', 'precipitation_mm', 'weather_condition']]
        external_data = external_data.merge(weather_subset, on='date', how='left')
        
        # Add promotion indicators
        promo_summary = self.promotions.groupby('date').agg({
            'discount_rate': 'mean',
            'promotion_type': 'count'
        }).rename(columns={'promotion_type': 'active_promotions'}).reset_index()
        ...
        external_data['active_promotions'] = external_data['active_promotions'].fillna(0)
        
        return external_data
    
    def _create_aggregated_datasets(self, sales_data: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        """Create aggregated datasets for different forecasting scenarios."""
        aggregated_datasets = {}
        
        # Monthly aggregation (most common for business forecasting)
        monthly_data = sales_data.groupby(['year', 'month', 'store_id', 'product_category']).agg({
            'daily_sales': 'sum',
            'is_weekend': 'mean',
            'is_holiday': 'mean',
            'holiday_factor': 'mean',
        ...
        weekly_data = sales_data.groupby(['year', 'week', 'store_id', 'product_category']).agg({
            'daily_sales': 'sum',
            'is_weekend': 'mean',
            'is_holiday': 'mean',
            'holiday_factor': 'mean',
        ...
        weekly_data['date'] = sales_data.groupby(['year', 'week'])['date'].first().reset_index()['date']
        ...
        store_daily = sales_data.groupby(['date', 'store_id']).agg({
            'daily_sales': 'sum',
            'is_weekend': 'first',
            'is_holiday': 'first',
            'holiday_factor': 'mean',
        ...
        category_daily = sales_data.groupby(['date', 'product_category']).agg({
            'daily_sales': 'sum',
            'is_weekend': 'first',
            'is_holiday': 'first',
            'holiday_factor': 'mean',
        ...
        category_daily = category_daily.rename(columns={'daily_sales': 'total_category_sales'})
    ...
    def _create_metadata(self) -> Dict:
        ...
        return {
            ...
            'data_structure': {
                ...
                'external_factors': [
                    'holidays', 'economic_indicators', 'weather', 'promotions'
                ]
            },
            'business_scenarios': {
                'seasonal_patterns': {
