...
class TimeSeriesFeatureEngineer:
    ...
    def fit(self, data: pd.DataFrame, 
            external_data: Optional[pd.DataFrame] = None) -> 'TimeSeriesFeatureEngineer':
        """
        ...
        print(f"Feature engineering fitted. Generated {len(self.feature_names_)} features.")
    ...
    def transform(self, data: pd.DataFrame,
        ...
        if self.scaler_ is not None:
            numeric_cols = features_encoded.select_dtypes(include=[np.number]).columns
            if len(numeric_cols) > 0:
                features_encoded[numeric_cols] = self.scaler_.transform(features_encoded[numeric_cols])
        
        # Apply feature selection
        if self.feature_selector_ is not None:
            selected_features = self.feature_selector_.transform(features_encoded)
            selected_feature_names = [self.feature_names_[i] for i in self.feature_selector_.get_support(indices=True)]
            features_encoded = pd.DataFrame(selected_features, 
    ...
    def _prepare_data(self, data: pd.DataFrame, 
        ...
        if external_data is not None:
            if self.date_col in external_data.columns:
                ...
                external_clean[self.date_col] = pd.to_datetime(external_clean[self.date_col])
                data_clean = pd.merge(data_clean, external_clean, on=self.date_col, how='left')
            else:
    ...
    def _generate_all_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Trend features
        trend_features = self._create_trend_features(data[self.target_col], data[self.date_col])
    ...
    def engineer_features(self, target_series: pd.Series, 
                         external_factors: pd.DataFrame = None) -> pd.DataFrame:
        """
        ...
        if external_factors is not None:
            data = data.merge(external_factors, left_on=self.date_col, 
    ...
    def _create_rolling_features(self, target: pd.Series) -> pd.DataFrame:
        ...
        for window in self.rolling_windows:
            ...
            features[f'rolling_mean_{window}'] = target.rolling(window=window, min_periods=1).mean()
            features[f'rolling_std_{window}'] = target.rolling(window=window, min_periods=1).std()
            ...
            features[f'rolling_median_{window}'] = target.rolling(window=window, min_periods=1).median()
            ...
            # Rolling percentiles
            features[f'rolling_q25_{window}'] = target.rolling(window=window, min_periods=1).quantile(0.25)
            features[f'rolling_q75_{window}'] = target.rolling(window=window, min_periods=1).quantile(0.75)
            
            # Position relative to rolling statistics
            features[f'target_vs_rolling_mean_{window}'] = target / features[f'rolling_mean_{window}']
            features[f'target_rolling_zscore_{window}'] = (
                (target - features[f'rolling_mean_{window}']) / features[f'rolling_std_{window}']
            )
        
        return features
    
    def _create_trend_features(self, target: pd.Series, dates: pd.Series) -> pd.DataFrame:
    ...
    def _create_holiday_features(self, dates: pd.Series) -> pd.DataFrame:
        ...
        for holiday_name, (month, day) in holidays.items():
            ...
            for offset in [-3, -2, -1, 1, 2, 3]:
                holiday_dates = pd.to_datetime([f'{year}-{month:02d}-{day:02d}' 
    ...
    def _create_change_point_features(self, target: pd.Series) -> pd.DataFrame:
        ...
        for window in [14, 30, 60]:
            if len(target) > window * 2:
                ...
                historical_mean = target.rolling(window=window, min_periods=1).mean().shift(window//2)
                historical_std = target.rolling(window=window, min_periods=1).std().shift(window//2)
                
                change_zscore = (recent_values - historical_mean) / (historical_std + 1e-8)
        ...
        for i, feat1 in enumerate(key_features):
            for feat2 in key_features[i+1:]:
                if feat1 in features.columns and feat2 in features.columns:
                    ...
                    if (features[feat1].dtype in [np.float64, np.int64] and 
                        features[feat2].dtype in [np.float64, np.int64]):
    ...
    def _handle_missing_values(self, features: pd.DataFrame, 
    ...
    def _fit_feature_selector(self, features: pd.DataFrame, target: pd.Series) -> None:
        ...
        if len(valid_features) > 0 and len(target_clean) > 0:
            ...
            self.feature_importance_ = dict(zip(
                valid_features.columns[selected_mask],
                feature_scores[selected_mask]
            ))
...
def create_forecasting_features(data: pd.DataFrame,
                               target_col: str = 'daily_sales',
                               date_col: str = 'date',
                               external_data: Optional[pd.DataFrame] = None,
                               config: Dict[str, Any] = None) -> Tuple[pd.DataFrame, TimeSeriesFeatureEngineer]:
    """
...
def analyze_feature_importance(engineer: TimeSeriesFeatureEngineer,
