...
class TestHybridForecastingModel(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.engineered_features = cls.feature_engineer.engineer_features(
            cls.sales_series, 
            external_factors=cls.external_factors
        )
        
        # Split data
        split_date = '2023-09-01'
        cls.train_data = cls.engineered_features[cls.engineered_features.index < split_date].copy()
        cls.test_data = cls.engineered_features[cls.engineered_features.index >= split_date].copy()
        ...
        print(f"Test data prepared: {len(cls.X_train)} training samples, {len(cls.X_test)} test samples")
    ...
    def test_model_initialization(self):
        ...
        self.assertIn(self.model.ensemble_strategy, ['simple_average', 'horizon_weighted', 'confidence_weighted'])
    ...
    def test_model_prediction(self):
        ...
        self.assertTrue(all(p >= 0 for p in predictions))  # Sales should be non-negative
    ...
    def test_prediction_accuracy(self):
        ...
        self.assertLess(mape, 50, "MAPE should be less than 50% for reasonable forecast")
        
        # Test correlation
        correlation = np.corrcoef(self.y_test, predictions)[0, 1]
        self.assertGreater(correlation, 0.3, "Predictions should have positive correlation with actual values")
    ...
    def test_confidence_intervals(self):
        ...
        try:
            confidence_intervals = self.model.predict_with_confidence(self.X_test, confidence_level=0.95)
            ...
            self.assertTrue(all(confidence_intervals['upper'] >= confidence_intervals['lower']))
    ...
    def test_reproducibility(self):
        ...
        self.assertTrue(np.mean(relative_diff) < 0.01, "Models should produce reproducible results")
...
class TestTimeSeriesValidation(unittest.TestCase):
    ...
    def test_walk_forward_validation(self):
        ...
        # Validate results
        self.assertGreater(validation_steps, 0, "Should complete at least one validation step")
        self.assertLess(np.mean(all_errors), 100, "Average MAPE should be reasonable")
        
        print(f"Walk-forward validation completed {validation_steps} steps, average MAPE: {np.mean(all_errors):.2f}%")
    
    def test_temporal_consistency(self):
        """Test temporal consistency of predictions."""
        # Split data
        split_date = '2023-09-01'
        train_data = self.engineered_features[self.engineered_features.index < split_date].copy()
        test_data = self.engineered_features[self.engineered_features.index >= split_date].copy()
        ...
        # Train model
        model = HybridForecastingModel(lightgbm_config={'verbose': -1, 'n_estimators': 20})
        model.fit(X_train, y_train)
        ...
        self.assertLess(extreme_changes.sum() / len(daily_changes), 0.1, 
        ...
        if len(pred_series) > 7:
            recent_correlation = pred_series.rolling(7).corr(pred_series.shift(1)).mean()
            if not np.isnan(recent_correlation):
                self.assertGreater(recent_correlation, 0, 
    ...
    def test_seasonal_pattern_preservation(self):
        ...
        model = HybridForecastingModel(lightgbm_config={'verbose': -1, 'n_estimators': 30})
        model.fit(X, y)
        ...
        if not np.isnan(weekly_correlation):
            self.assertGreater(weekly_correlation, 0.3, 
    ...
    def test_forecast_horizon_performance(self):
        ...
        self.assertGreaterEqual(len(horizon_errors), 2, "Should test multiple forecast horizons")
        ...
        if len(horizon_errors) >= 2:
            ...
            self.assertLess(max(errors) / min(errors), 3.0, 
...
class TestModelComponents(unittest.TestCase):
    ...
    def test_prophet_component(self):
        ...
        try:
            ...
            # Make predictions
            future_dates = pd.date_range(self.sales_data.index[-1] + pd.Timedelta(days=1), 
                                       periods=30, freq='D')
...
class TestBusinessLogic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.actual = pd.Series(1000 + np.random.normal(0, 100, len(dates)), index=dates)
        self.forecast = pd.Series(1000 + np.random.normal(0, 80, len(dates)), index=dates)
    ...
    def test_forecast_evaluator(self):
        ...
        expected_keys = ['accuracy_metrics', 'business_insights', 'performance_analysis']
...
class TestDataQuality(unittest.TestCase):
    ...
    def test_feature_engineering(self):
        ...
        # Generate test data
        dates = pd.date_range('2023-01-01', '2023-12-31', freq='D')
        sales_series = pd.Series(1000 + np.random.normal(0, 100, len(dates)), index=dates)
