...
class TestEndToEndPipeline(unittest.TestCase):
    ...
    def test_multi_store_pipeline(self):
        ...
        # Validate results
        self.assertGreater(len(results), 0, "Should successfully process at least one store")
        
        for store, result in results.items():
            self.assertLess(result['mape'], 100, f"Store {store} MAPE too high: {result['mape']:.2f}%")
            self.assertGreater(result['predictions_count'], 0, f"Store {store} has no predictions")
    ...
    def test_model_persistence(self):
        ...
        print(f"Model persistence test passed: {len(original_predictions)} predictions matched")
    ...
    def test_feature_engineering_robustness(self):
        ...
        base_sales = pd.Series(1000 + np.random.normal(0, 100, len(base_dates)), index=base_dates)
            ...
            try:
                ...
                for col in numeric_features.columns:
                    if col != 'target':  # Target might have missing values
                        finite_values = numeric_features[col].replace([np.inf, -np.inf], np.nan).dropna()
                        if len(finite_values) > 0:
                            self.assertTrue(finite_values.std() < finite_values.mean() * 10, 
    ...
    def test_business_intelligence_integration(self):
        ...
        evaluation = self.evaluator.evaluate_forecast(y_test, predictions)
        dashboard = self.business_intelligence.generate_executive_dashboard(y_test, predictions)
        ...
        for metric, value in health.items():
            if isinstance(value, (int, float)):
                self.assertGreaterEqual(value, 0, f"Health metric {metric} should be non-negative")
                self.assertLessEqual(value, 10, f"Health metric {metric} should be <= 10")
...
class TestPerformanceBenchmarks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        split_date = '2023-09-01'
        train_data = self.engineered_features[self.engineered_features.index < split_date]
        test_data = self.engineered_features[self.engineered_features.index >= split_date]
    ...
    def test_training_performance(self):
        ...
        self.assertLess(training_time, 300, f"Training took too long: {training_time:.2f} seconds")
        
        print(f"Training performance: {training_time:.2f} seconds for {len(self.X_train)} samples")
    ...
    def test_prediction_performance(self):
        ...
        print(f"Prediction performance: {predictions_per_second:.2f} predictions/second")
        print(f"Prediction time: {prediction_time:.4f} seconds for {len(self.X_test)} samples")
    ...
    def test_memory_usage(self):
        ...
        self.assertLess(training_memory_delta, 1000, 
                       f"Training memory usage too high: {training_memory_delta:.2f} MB")
        self.assertLess(prediction_memory_delta, 100, 
                       f"Prediction memory usage too high: {prediction_memory_delta:.2f} MB")
        
        print(f"Memory usage - Training: +{training_memory_delta:.2f} MB, "
