...
class TestSalesDataGenerator(unittest.TestCase):
    ...
    def test_full_dataset_generation(self):
        ...
        expected_combinations = len(self.generator.stores) * len(self.generator.product_categories)
...
class TestFeatureEngineer(unittest.TestCase):
    ...
    def test_lag_features(self):
        ...
        # Check lag values
        self.assertTrue(pd.isna(lag_features['sales_lag_1'].iloc[0]))  # First value should be NaN
        self.assertAlmostEqual(
            lag_features['sales_lag_1'].iloc[1], 
            self.sales_series.iloc[0], 
            places=6
        )
        
        # Check 7-day lag
        self.assertTrue(pd.isna(lag_features['sales_lag_7'].iloc[6]))  # 7th value should be NaN
        self.assertAlmostEqual(
            lag_features['sales_lag_7'].iloc[7], 
            self.sales_series.iloc[0], 
            places=6
        )
    ...
    def test_rolling_features(self):
        ...
        rolling_features = self.feature_engineer._create_rolling_features(self.sales_series)
        ...
        self.assertTrue(pd.isna(rolling_features['sales_ma_7'].iloc[6]))  # First 6 should be NaN
        
        # Manually calculate 7-day MA for verification
        expected_ma = self.sales_series.iloc[:7].mean()
        self.assertAlmostEqual(
            rolling_features['sales_ma_7'].iloc[6], 
            expected_ma, 
            places=6
        )
    ...
    def test_seasonal_features(self):
        ...
        seasonal_features = self.feature_engineer._create_seasonal_features(self.sales_series)
        ...
        if january_mask.any():
            ...
            self.assertEqual(len(jan_sin_values), 1)  # Should be same for all January days
    ...
    def test_trend_features(self):
        ...
        self.assertEqual(trend_values.iloc[-1], len(self.sales_series) - 1)  # Should end at length-1
        ...
        expected_pct_change = (self.sales_series.iloc[1] - self.sales_series.iloc[0]) / self.sales_series.iloc[0]
    ...
    def test_external_factor_integration(self):
        ...
        np.testing.assert_array_equal(
            features['temperature'].values,
            self.external_factors['temperature'].values
        )
    ...
    def test_full_feature_engineering(self):
        ...
        np.testing.assert_array_equal(
            features['target'].dropna().values,
            self.sales_series.dropna().values
        )
        ...
        self.assertGreater(len(numeric_columns), 10)  # Should have many numeric features
        
        # Check for reasonable feature counts
        self.assertGreater(len(features.columns), 15)  # Should have substantial feature set
    ...
    def test_feature_selection(self):
        ...
        if hasattr(selective_engineer, 'max_features') and selective_engineer.max_features:
            # Allow some flexibility for target and essential features
            self.assertLessEqual(len(features.columns), selective_engineer.max_features + 5)
...
class TestForecastEvaluator(unittest.TestCase):
    ...
    def test_perfect_forecast(self):
        ...
        metrics = self.evaluator.calculate_accuracy_metrics(self.actual, perfect_forecast)
    ...
    def test_business_insights(self):
        ...
        insights = self.evaluator.calculate_business_insights(self.actual, self.forecast)
        ...
        self.assertIn(bias['bias_direction'], ['under_forecast', 'over_forecast', 'unbiased'])
    ...
    def test_evaluate_forecast(self):
        ...
        expected_sections = ['accuracy_metrics', 'business_insights', 'performance_analysis']
    ...
    def test_edge_cases(self):
        ...
        metrics = self.evaluator.calculate_accuracy_metrics(constant_actual, constant_forecast)
        ...
        with self.assertRaises((ValueError, IndexError)):
            self.evaluator.calculate_accuracy_metrics(
                self.actual, 
                self.forecast.iloc[:-10]  # Shorter forecast
            )
        ...
        try:
            metrics = self.evaluator.calculate_accuracy_metrics(zero_actual, self.forecast)
...
class TestBusinessIntelligence(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.actual = pd.Series(base_sales * weekly_pattern * monthly_growth, index=self.dates)
    ...
    def test_core_kpis(self):
        ...
        # Check expected KPIs
        expected_kpis = ['total_revenue', 'average_daily_sales', 'forecast_accuracy', 'revenue_growth']
    ...
    def test_business_health_scoring(self):
        ...
        expected_metrics = ['overall_health_score', 'forecast_reliability', 'trend_stability']
        for metric in expected_metrics:
    ...
    def test_executive_dashboard(self):
        ...
        expected_sections = ['core_kpis', 'business_health', 'executive_summary', 'recommendations']
        for section in expected_sections:
    ...
    def test_roi_analysis(self):
        ...
        expected_metrics = ['forecast_value', 'cost_of_forecast_errors', 'accuracy_improvement_value']
        for metric in expected_metrics:
    ...
    def test_edge_cases(self):
        ...
        try:
            dashboard = self.bi.generate_executive_dashboard(short_actual, short_forecast)
