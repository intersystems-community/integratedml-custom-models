...
try:
    ...
    logger.warning(
        "IRIS Python package not available. Using HTTP-based approach for basic operations."
    )
...
class IRISConnection:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized IRIS connection to {self.host}:{self.port}/{self.namespace}"
        )
    ...
    def get_native_connection(self):
        ...
        if not IRIS_AVAILABLE:
            raise RuntimeError(
                "Native IRIS package not available. Install 'intersystems-iris' package."
            )
    ...
    def _execute_http_sql(
        ...
        try:
            ...
            response = requests.post(
                url, json=payload, auth=auth, timeout=self.connection_timeout
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"HTTP SQL execution failed: {response.status_code} - {response.text}"
                )
                return []
    ...
    def get_integratedml_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get model count
            try:
                models_result = self.execute_sql(
                    "SELECT COUNT(*) FROM INFORMATION_SCHEMA.ML_MODELS"
                )
