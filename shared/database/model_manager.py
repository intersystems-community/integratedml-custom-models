...
class ModelManager:
    ...
    def predict(
        ...
        try:
            ...
            values_clause = ', '.join([f"'{v}'" if isinstance(v, str) else str(v) for v in values])
    ...
    def _update_model_status(self, model_name: str, status: str):
        ...
        WHERE model_name = ?
        """
        self.conn.execute_query(update_sql, {'status': status, 'model_name': model_name})
    ...
    def _log_prediction(
        ...
        try:
            ...
            self.conn.execute_query(log_sql, {
                'model_name': model_name,
