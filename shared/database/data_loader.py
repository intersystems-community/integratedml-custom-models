...
class DataLoader:
    ...
    def load_fraud_detection_data(self, num_samples: int = None) -> bool:
        ...
        """
        try:
            num_samples = num_samples or int(os.getenv('FRAUD_DETECTION_SAMPLES', '50000'))
    ...
    def _generate_credit_risk_data(self, num_samples: int) -> List[Dict[str, Any]]:
        ...
        for i in range(num_samples):
            ...
            data.append({
                ...
                'loan_purpose': np.random.choice(['home', 'auto', 'personal', 'business', 'education']),
                'home_ownership': np.random.choice(['own', 'rent', 'mortgage']),
                'annual_income': round(income, 2),
                'verification_status': np.random.choice(['verified', 'not_verified', 'source_verified']),
    ...
    def _generate_fraud_detection_data(self, num_samples: int) -> List[Dict[str, Any]]:
        ...
        for i in range(num_samples):
            ...
            # Location
            location_country = np.random.choice(['US', 'CA', 'UK', 'FR', 'DE', 'JP', 'OTHER'], p=[0.7, 0.1, 0.05, 0.05, 0.03, 0.02, 0.05])
            ...
            fraud_probability = min(0.9, fraud_indicators * 0.15 + np.random.random() * 0.1)
            is_fraud = 1 if np.random.random() < fraud_probability and np.random.random() < fraud_rate * 10 else 0
    ...
    def _generate_sales_forecasting_data(self, num_days: int) -> List[Dict[str, Any]]:
        ...
        products = [
            {'id': i, 'category': np.random.choice(['Electronics', 'Clothing', 'Home', 'Books', 'Sports'])}
        ...
        for day_offset in range(num_days):
            ...
            for product in products:
                ...
                base_sales = np.random.lognormal(6, 1) * seasonal_factor * trend_factor * weekend_factor
    ...
    def _insert_fraud_detection_batch(self, batch_df: pd.DataFrame):
        ...
        for _, row in batch_df.iterrows():
            self.conn.execute_query(insert_sql, {
                ...
                'days_since_last_transaction': float(row['days_since_last_transaction']),
