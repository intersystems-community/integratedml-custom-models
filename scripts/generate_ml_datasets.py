...
def generate_transactions(count: int = 2000) -> List[Tuple]:
    ...
    for i in range(1, count + 1):
        ...
        data.append(
            (i, round(amount, 2), merchant, hour, day_of_week, user_age, is_fraud)
        )
...
def generate_sales_data(count: int = 1500) -> List[Tuple]:
    ...
    for i in range(1, count + 1):
        ...
        data.append(
            (i, sale_date.isoformat(), round(amount, 2), quantity, category, quarter)
        )
...
def generate_dna_sequences(count: int = 800) -> List[Tuple]:
    ...
    for i in range(1, count + 1):
        ...
        location = (
            random.choice(chromosomes)
            if species_name in ["Human", "Mouse"]
            else f"Scaffold {random.randint(1, 100)}"
        )
...
def main():
    ...
    datasets = [
        (
            "CreditApplications",
            generate_credit_applications(1000),
            "(id, income, credit_history_length, debt_to_income_ratio, employment_status, loan_amount, risk_score)",
        ),
        (
            "Transactions",
